...
import { addExp, subtractExp, applyPenalty, incrementQuestCount, createDefaultPlayer, getQuestsPerDay, isWeeklySkipAvailable, } from './game-engine';
import { rollArtifactReward, calcExpMultiplier, calcPenaltyReduction } from './artifact-data';
...
async function handleArtifactRewardPress() {
    ...
    if (pendingRankUp) {
        ...
        const lvl = pendingLevelUp.oldLevel;
        pendingLevelUp = null;
        levelIdx = 0;
        currentScreen = 'levelUp';
...
main().catch(async (err) => {
    ...
    try {
        await display?.update(display.buildError(msg));
    }
    catch { }
});
