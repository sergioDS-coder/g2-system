...
export async function renderQuestImages(templateId, info) {
...
export async function renderWildQuestImage() {
...
function drawWildQuestCard(ctx, w, h) {
    ...
    ctx.lineWidth = 2;
    ctx.strokeStyle = '#fff';
    for (const [x, y, dx, dy] of [[16, 56, 1, 1], [w - 16, 56, -1, 1], [16, h - 56, 1, -1], [w - 16, h - 56, -1, -1]]) {
...
export async function renderWelcomeImage(rank) {
...
function drawCardOverlay(ctx, info) {
...
function renderRankCard(rank) {
    ...
    const STAR_COUNT = { F: 1, E: 2, D: 3, C: 4, B: 5, A: 6, S: 7, SS: 8, SSS: 9 };
...
function canvasToImageBytes(src, sx, sy, w, h) {
    const c = document.createElement('canvas');
    c.width = w;
    c.height = h;
    const ctx = c.getContext('2d');
...
// ── Illustrations ──────────────────────────────────────────────────────────────
function drawRunning(ctx, _w, _h) {
    const cx = 108, top = 38;
    // head
...
function drawWalking(ctx, _w, _h) {
    const cx = 90, top = 28;
    // head
    head(ctx, cx, top + 22, 22);
    // torso (upright)
...
function drawCrunches(ctx, _w, _h) {
    const cy = 165;
...
function drawMeditation(ctx, _w, _h) {
    const cx = 90, cy = 158;
    // aura rings
...
function drawWriting(ctx, _w, _h) {
    // paper
    ctx.globalAlpha = 0.2;
...
function drawNoScreen(ctx, _w, _h) {
    const cx = 90, cy = 118;
    const mw = 144, mh = 92;
    // monitor frame (white)
...
function drawSleep(ctx, w, _h) {
    ...
    ctx.fillStyle = '#fff';
    // stars
    for (const [sx, sy, sr] of [[145, 22, 8], [162, 58, 5], [128, 78, 4]]) {
        head(ctx, sx, sy, sr);
    }
